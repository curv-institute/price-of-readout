...
a sparse-superposition family), and we determine in closed form what
beating a floor must cost: for the superposition family, exactly the
budgets with an unshared stored-parameter budget of at least $5$ and
...
controlled spurious color cue) test the predictions. The registered pass/fail criteria are
met; one descriptive prediction fails and usefully exposes cue
cannibalization. Cue-reliant models beat a cue-decorrelated
...
readout. The two failure modes the pair separates also appear on a
pretrained language model (Pythia-410m): low-bit weight quantization is
information-borne, with an irreducible refit floor and a sharp
four-to-three-bit onset, while distribution shift is interface-borne,
recoverable in proportion to how far out-of-distribution the shift is.
\end{minipage}%
\end{center}%
\vskip 1.6em
}]
...
frozen number is what deployment will experience; the refit number is
...
\begin{theorem}[lawful compression is free]
...
\begin{theorem}[unlawful compression strictly costs]
...
\begin{remark}[lawful compression can strictly degrade the interface]
...
\begin{corollary}[$K$ and $E$ may drop freely under lawfulness]
...
compression $S$ (sufficient by construction); and the unlawful
compression $N$ (insufficient by construction, since
...
\paragraph{The masking signature, first exhibition.} The unlawful arm
...
the analytic in-distribution excess of the unlawful arm over the lawful arm
...
insufficiency by construction); the \textbf{lawful} arm trains
...
but carries zero label information, so the learned representation
cannot exploit it. Shifts re-draw only the indicator, leaving image
...
(Figure~\ref{fig:frozen-refit}). The frozen number is
what deployment will experience; the refit number is what the
representation still supports; their difference is the price of the
...
head recovers a meaningful but bounded fraction ($30$--$57\%$, the
...
\hline
8 & 0.001 & --- & 0.001 & free\\
4 & 0.389 & 0.222 & \textbf{0.166} & 57\%\\
3 & 1.990 & 0.601 & \textbf{1.390} & 30\%\\
2 & 2.451 & 0.894 & \textbf{1.557} & 36\%\\
\hline
\end{tabular}
\caption{Weight quantization on Pythia-410m (bits/byte, in-distribution;
reference is the 16-bit head-refit score). Head-refit recovers the
...
to $\approx 0.27$ bits/byte for clearly-OOD languages
(Table~\ref{tab:pythia-ood}; Figure~\ref{fig:pythia}b; Pearson $0.93$, Spearman $0.86$, $n=8$,
...
Finnish & 1.57 & $+0.031$\\
Yoruba & 2.22 & $\mathbf{+0.295}$\\
Swahili & 2.34 & $\mathbf{+0.252}$\\
Welsh & 2.46 & $\mathbf{+0.274}$\\
\hline
\end{tabular}
\caption{Distribution shift on Pythia-410m (bits/byte). The
head-refit-recoverable (interface-borne) part is near zero for
in-distribution/mild shifts and grows to a quarter-bit for genuinely-OOD
languages; recovery tracks OOD distance (Pearson $0.93$). Same-script-width
...
\caption{External replication on Pythia-410m: the two failure modes the paper
isolates on systems with exact ground truth, separated by the same
frozen-versus-refit pair. \textbf{(a)} Weight quantization (in-distribution;
...
grows with OOD distance, the interface-borne signature (Pearson $0.93$,
...
activation quantization is a distinct mixed locus ($\approx 40\%$
interface-borne). The boundary condition is the honest part: the shift leg is
...
the wild is a separate (and hard) problem. (Experiment~3's lawful arm is
sufficiency-by-construction \emph{for the semantic task}: the tint is
made uninformative, not inferred to be.)
...
{\small\texttt{mlr-proof-program/experiments/track3\_gate2/}}:\\
...
{\small\texttt{mlr-proof-program/experiments/track4\_gateB/}}:\\
pre-registration commit\\
...
{\small\texttt{mlr-proof-program/experiments/}\allowbreak
\texttt{expB\_masking\_real/}}:\\
pre-registration commit\\
...
\texttt{cd mlr-proof-program/experiments/track3\_gate2}\\
...
\begin{flushleft}\footnotesize
\texttt{cd mlr-proof-program/experiments/track4\_gateB}\\
...
{\itshape \hspace*{1.5em}(all machine-checked enumerations of
Appendix~\ref{app:enum}; asserts every claim)}\\
\texttt{python3 ../kef\_frontier/superposition\_min\_e.py}\\
...
\begin{flushleft}\footnotesize
\texttt{cd mlr-proof-program/experiments/expB\_masking\_real}\\
...
by hand into the script. Regenerate with
\begin{center}
\texttt{python3 figures/make\_figures.py}.
\end{center}
The paper then builds with two
passes of \texttt{pdflatex -interaction=nonstopmode main.tex} in the
directory
\begin{center}
\texttt{mlr-proof-program/papers/}\\
\texttt{lawful-compression-readout-geometry/}.
\end{center}
...
\texttt{experiments/\allowbreak track4\_gateB/\allowbreak
...
\texttt{experiments/\allowbreak kef\_frontier/\allowbreak
...
(under \texttt{experiments/track4\_gateB/}) executes it from scratch
...
\texttt{experiments/kef\_frontier/}; Python standard library only,
...
reduces to one of ten interval conditions, each empty over the
...
corresponding names in the reproduction archive (locations relative to
\texttt{mlr-proof-program/}). The archived pre-registration and results
...
\texttt{experiments/track3\_gate2/} \\
Experiment 2 (readout ladder) & ``track 4 / gate B'' &
\texttt{experiments/track4\_gateB/} \\
Experiment 3 (masking at scale) & ``experiment B''
(\texttt{expB-masking-real-v1}) & \texttt{experiments/expB\_masking\_real/} \\
