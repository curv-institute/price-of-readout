...
interface-borne; protocol and per-bit numbers in
...
$512$, under \texttt{bfloat16} autocast. The in-distribution
decomposition uses the WikiText-103 test split; each shift cell draws
its text from that shift's domain. Within a cell the evaluation corpus
is the first $2$\,MB and the refit corpus the first $8$\,MB of the same
source file, so the refit text \emph{contains} the evaluation text as a
subset. The reported recovery is therefore how much an in-domain head
re-fit closes the frozen interface's gap, not generalization to unseen
held-out text; this is adequate for the qualitative
information-borne-versus-interface-borne contrast the leg is making, but
is weaker than the disjoint-split protocol of the pre-registered
experiments and is one reason the leg is reported as a post-hoc check
rather than a gated result.
...
$16$-bit head-refit loss ($0.861$ bits/byte) is the reference against
...
$\approx\!40\%$-interface-borne activation-quantization result cited in
Section~\ref{sec:pythia} uses the same frozen/head-refit in-distribution
protocol, with per-tensor dynamic symmetric fake quantization applied to
...
head-refit reference $\text{aref} = 0.8605$ bits/byte
(Table~\ref{tab:pythia-actquant}); its mean of $\approx\!41\%$ places
...
Table~\ref{tab:pythia-quant}). ``total'' is the frozen loss above the
$16$-bit-activation head-refit reference $0.8605$; ``recoverable'' is
